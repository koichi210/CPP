// Menu.cpp : アプリケーションのエントリ ポイントを定義します。
//
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDM_ADD_SUB_MENU :
			CreateMySubMenu(hWnd);
			break;

		case IDM_DEL_SUB_MENU :
			DeleteMySubMenu(hWnd);
			break;
...
VOID CreateMyMenu(HWND hWnd)
{
	...
	// トップメニューの取得
	HMENU hTestArea = GetSubMenu( hMenu, 2 );

	// メニュー追加
	AppendMenu(hTestArea, MF_BYPOSITION ,1 ,_T("Create Menu!!"));
}
...
VOID DeleteMyMenu(HWND hWnd)
{
	...
	// メニュー削除
	DeleteMenu(hTestArea, 0, MF_BYPOSITION);
}
...
VOID CreateMySubMenu(HWND hWnd)
{
	...
	// サブメニューの取得
	HMENU hSubMenu = GetSubMenu( hTestArea, 1 );

	// メニュー追加
	AppendMenu(hSubMenu, MF_BYPOSITION ,1 ,_T("Create SubMenu!!"));
}
