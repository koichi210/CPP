================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: VersionCompare プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した VersionCompare には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには VersionCompare アプリケーションを構成している各ファイ
ルの概要説明が含まれます。

VersionCompare.vcproj
...
VersionCompare.h
   ...
   CVersionCompareApp アプリケーション クラスを宣言します。

VersionCompare.cpp
   これは、アプリケーション クラス CVersionCompareApp を含むメイン アプリケー
   ション ソース ファイルです。
...
res\VersionCompare.ico
   これは、アプリケーションのアイコンとして使用されるアイコンファイルです。この
   アイコンはメイン リソース ファイル VersionCompare.rc に含まれていま
   す。

res\VersionCompare.rc2
...
VersionCompareDlg.h, VersionCompareDlg.cpp - ダイアログ
   これらのファイルは CVersionCompareDlg クラスを含みます。このクラスはアプ
   リケーションのメイン ダイアログの動作を定義します。ダイアログ テンプレートは
   Microsoft Visual C++ で編集可能な VersionCompare.rc に含まれます。
...
StdAfx.h, StdAfx.cpp
   ...
   VersionCompare.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
