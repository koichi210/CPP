// BackUpDiffDlg.cpp : 実装ファイル
//

#include <shlwapi.h>
#include <shlobj.h>
#include "stdafx.h"
#include "BackUpDiff.h"
#include "BackUpDiffDlg.h"
...
CBackUpDiffDlg::CBackUpDiffDlg(CWnd* pParent /*=NULL*/)
	: CDialog(CBackUpDiffDlg::IDD, pParent)
{
...
void CBackUpDiffDlg::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
	DDX_Control(pDX, IDC_LISTCTRL, m_listctl);
}

BEGIN_MESSAGE_MAP(CBackUpDiffDlg, CDialog)
	ON_WM_SYSCOMMAND()
	ON_WM_PAINT()
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
...
// CBackUpDiffDlg メッセージ ハンドラ

BOOL CBackUpDiffDlg::OnInitDialog()
{
...
void CBackUpDiffDlg::OnSysCommand(UINT nID, LPARAM lParam)
{
...
//  システムがこの関数を呼び出します。
HCURSOR CBackUpDiffDlg::OnQueryDragIcon()
{
	return static_cast<HCURSOR>(m_hIcon);
}


void CBackUpDiffDlg::OnLvnItemchangedList(NMHDR *pNMHDR, LRESULT *pResult)
{
...
void CBackUpDiffDlg::InsertListCtrl(LV_COLUMN lvCol, int idx, CString name)
{
	lvCol.iSubItem = idx;
...
void CBackUpDiffDlg::OnBackupStart()
{
	char str[MAX_PATH];
	BOOL bErrFind = FALSE;
...
BOOL CBackUpDiffDlg::GetDirectory(TCHAR * dir) 
{
...
BOOL CBackUpDiffDlg::BackUpProc()
{
	BOOL bReturn = FALSE;
...
void CBackUpDiffDlg::OnBrowseSrc()
{
...
void CBackUpDiffDlg::OnBrowseDest()
{
	TCHAR pathName[MAX_PATH] = { 0 };
...
void CBackUpDiffDlg::OnEnChangeEditSrc()
{
	char str[MAX_PATH];
...
void CBackUpDiffDlg::OnEnChangeEditDst()
{
	char str[MAX_PATH];
...
void CBackUpDiffDlg::OnBnClickedEnableBK()
{
	BOOL bOn;
...
void CBackUpDiffDlg::OnBnClickedSubdir()
{
	BOOL bOn;
...
void CBackUpDiffDlg::OnBnClickedOverWrite()
{
	BOOL bOn;
...
void CBackUpDiffDlg::UpdateEnableBK(BOOL bChk, int idx)
{
	CString str;
...
void CBackUpDiffDlg::UpdateSubDirectory(DWORD opt, int idx)
{
	CString str;
	int nItem = GetIdx(idx);
...
void CBackUpDiffDlg::UpdateDiffFile(DWORD opt, int idx)
{
	CString str;
	int nItem = GetIdx(idx);
...
void CBackUpDiffDlg::UpdateOverWrite(DWORD opt, int idx)
{
	CString str;
	int nItem = GetIdx(idx);
...
void CBackUpDiffDlg::UpdateSrcPath(char *str, int idx)
{
	int nItem = GetIdx(idx);
...
void CBackUpDiffDlg::UpdateDstPath(char *str, int idx)
{
	int nItem = GetIdx(idx);
...
void CBackUpDiffDlg::OnBnClickedAllClear()
{
	// バグってるらしい。。
...
void CBackUpDiffDlg::OnBnClickedSaveSetting()
{
...
BOOL CBackUpDiffDlg::WriteSetting()
{
	char path[MAX_PATH];
	BOOL bReturn = FALSE;
...
BOOL CBackUpDiffDlg::ReadSetting()
{
	char path[MAX_PATH];
	BOOL bReturn = FALSE;
	int idx=0;
...
void CBackUpDiffDlg::WriteBatchFile(CString cmd)
{
	char path[MAX_PATH];
	BOOL bReturn = FALSE;
...
int CBackUpDiffDlg::GetIdx(int idx)
{
	int nItem = idx;
...
BOOL CBackUpDiffDlg::IsBlankData(BACKUP *BkStruct)
{
	BOOL bReturn = FALSE;
...
void CBackUpDiffDlg::OnBnClickedEndNone()
{
	m_end = END_NONE;
}

void CBackUpDiffDlg::OnBnClickedEndApp()
{
	m_end = END_APP;
}

void CBackUpDiffDlg::OnBnClickedEndReboot()
{
	m_end = END_REBOOT;
}

void CBackUpDiffDlg::OnBnClickedEndShutdown()
{
	m_end = END_SHUTDOWN;
}

void CBackUpDiffDlg::OnCancel()
{
	m_FileOut = TRUE;
	BackUpProc();
