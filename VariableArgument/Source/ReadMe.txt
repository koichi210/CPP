================================================================================
MICROSOFT FOUNDATION CLASS ライブラリ: VariableArgument プロジェクトの概要
===============================================================================

アプリケーション ウィザードが作成した VariableArgument には Microsoft 
Foundation Class の基本的な使い方が示されています。アプリケーション作成のひな型
としてお使いください。

このファイルには VariableArgument アプリケーションを構成している各ファイ
ルの概要説明が含まれます。

VariableArgument.vcxproj
...
VariableArgument.vcxproj.filters
...
VariableArgument.h
   ...
   CVariableArgumentApp アプリケーション クラスを宣言します。

VariableArgument.cpp
   これは、アプリケーション クラス CVariableArgumentApp を含むメイン アプリケー
   ション ソース ファイルです。
...
res\VariableArgument.ico
   これは、アプリケーションのアイコンとして使用されるアイコンファイルです。この
   アイコンはメイン リソース ファイル VariableArgument.rc に含まれていま
   す。

res\VariableArgument.rc2
...
VariableArgumentDlg.h, VariableArgumentDlg.cpp - ダイアログ
   これらのファイルは CVariableArgumentDlg クラスを含みます。このクラスはアプ
   リケーションのメイン ダイアログの動作を定義します。ダイアログ テンプレートは
   Microsoft Visual C++ で編集可能な VariableArgument.rc に含まれます。
...
StdAfx.h, StdAfx.cpp
   ...
   VariableArgument.pch や既にコンパイルされた型のファイル StdAfx.obj を
   ビルドするために使用されます。
